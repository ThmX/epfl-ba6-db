...
object RepresentativeController extends Controller {

  def entity = Representative

  def template(list: List[(Athlete, Country)])(implicit flash: Flash): Html = views.html.representatives.apply(list)(flash)

  def routeList = routes.RepresentativeController.list
  def routeSave = routes.RepresentativeController.save
