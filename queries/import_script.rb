...
db = Mysql.new('localhost', 'root', '', 'db_project_group_12_new')
...
CSV.foreach("dataset/athletes.csv") do |row|
  begin
    ...
    puts "DEBUG (Athletes) : #{ex.message}" if @debug
  end
  i += 1
end
puts "Done!"
...
CSV.foreach("dataset/countries.csv") do |row|
  begin
    ...
    puts "DEBUG (Countries) : #{ex.message}" if @debug
  end
  i += 1
end
puts "Done!"
...
CSV.foreach("dataset/sports.csv") do |row|
  begin
    ...
    puts "DEBUG (Sports) : #{ex.message}" if @debug
  end
  i += 1
end
puts "Done!"
...
CSV.foreach("dataset/disciplines.csv") do |row|  
  if i > 0
    ...
    result.each_hash do |h| 
      begin
        db.query("INSERT INTO Disciplines (name, sport_id) VALUES ('#{row[0].gsub("'", "\\\\'")}', '#{h['id']}')")
      rescue => ex
        puts "DEBUG (Disciplines) : #{ex.message}" if @debug
      end
    end
  end
  i += 1
    ...
    if i > 0
      ...
      result.each_hash do |h|
        begin
          ...
          puts "DEBUG (Games) : #{ex.message}" if @debug
        end
      end
    end
    i += 1
