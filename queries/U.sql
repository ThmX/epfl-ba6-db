...
-- Checks if a country has defended it's title from the previous game.
...
p2.discipline_id = d2.id AND p2.country_id = c2.id AND p2.games_id = g2.id AND p2.ranking = 1 AND d1.id = d2.id AND g1.id != g2.id AND NOT EXISTS (
  SELECT *
  FROM games g
  WHERE g.year < g1.year AND g.year > g2.year
)
GROUP BY d1.id